...
export default function RenameSpaceModal({ isOpen, onClose, project, onRenamed }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await api.patch(`/workspaces/${workspaceId}/projects/${project.id}`, {
        name: trimmed,
      });
      onRenamed?.();
      onClose();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to rename space');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        <div className="px-6 pt-6 pb-4">
          <h2 id="rename-space-title" className="text-lg font-semibold text-gray-900">
            Rename space
          </h2>
          <p className="mt-1 text-sm text-gray-500">Update how this space appears in the sidebar.</p>
          <label className="mt-4 block text-sm font-medium text-gray-700" htmlFor="rename-space-input">
            Name
          </label>
          <input
